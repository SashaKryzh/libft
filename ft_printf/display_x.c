...
char		*x_alter(t_arg *arg, char *s, int len)
{
	char	*tmp;
	int		i;
...
char		*x_precision(t_arg *arg, char *s)
{
	char	*tmp;
	int		len;
	int		i;
...
int			x_padding(t_arg *arg, char pad, int i)
{
	int ret;
...
int			display_x(t_arg *arg, va_list ap, char spec)
{
	...
	MALCH((s = u_itoa_base(n, 16)));
	*s = arg->precision == 0 && n == 0 ? '\0' : *s;
	arg->alter = n == 0 ? 0 : arg->alter;
	s = x_precision(arg, s);
	ret = ft_strlen(s);
	if (arg->width > ret)
		ret += x_padding(arg, arg->pad, ret);
	ret += arg->alter ? 2 : 0;
